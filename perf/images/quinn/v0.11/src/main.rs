...
// ---------------------------------------------------------------------------
// ZeroReader — AsyncRead that yields zero bytes
// ---------------------------------------------------------------------------
...
impl AsyncRead for ZeroReader {
    fn poll_read(
        ...
        // Fill with zeros (ReadBuf is already zeroed by default)
...
async fn handle_stream(mut send: SendStream, mut recv: RecvStream) {
    ...
    if requested_bytes == 0 {
        ...
        let mut reader = ZeroReader.take(requested_bytes);
        let _ = tokio::io::copy(&mut reader, &mut send).await;
        let _ = send.finish();
    }
}
...
async fn run_upload_test(
    ...
    for i in 0..iterations {
        let conn = match dial(server_addr, client_config).await {
            Ok(c) => c,
            Err(e) => {
        ...
        // Send N zero bytes
        let mut reader = ZeroReader.take(bytes);
        if let Err(e) = tokio::io::copy(&mut reader, &mut send).await {
            eprintln!("Upload iteration {} copy failed: {e}", i + 1);
        }
        let _ = send.finish();
...
async fn run_download_test(
    ...
    for i in 0..iterations {
        ...
        let _ = tokio::io::copy(&mut recv, &mut discard).await;
...
async fn run_dialer(cfg: &Config) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let upload_stats =
        run_upload_test(server_addr, &client_config, cfg.upload_bytes, cfg.upload_iters).await;
    ...
    let latency_stats =
        run_latency_test(server_addr, &client_config, cfg.latency_iters).await;
